...
const Order = () => {
  ...
  return (
    <AccountLayout title='My orders' subtitle='Track your recent purchases and order status.'>
      {ordersLoading ? (
        <div className='rounded-lg border border-gray-200 bg-white p-5 text-sm text-gray-600'>Loading orders...</div>
      ) : null}

      {ordersError ? (
        <div className='rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700'>{ordersError}</div>
      ) : null}
